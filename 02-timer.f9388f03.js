...
const options = {
    ...
    onClose (selectedDates) {
        ...
        if (timeToEnd > 0) buttonEl.disabled = false;
        else {
...
const fp = (0, _flatpickrDefault.default)(inputEl, options);
buttonEl.disabled = true;
buttonEl.addEventListener("click", startTimer);
function startTimer() {
    (0, _notiflixDefault.default).Notify.success("TIMER STARTED");
    ...
    const timerID = setInterval(()=>{
        ...
        if (timeToEnd <= 0) {
            ...
            (0, _notiflixDefault.default).Notify.info("THE END)))");
        }
        changeText();
    }, 1000);
}
